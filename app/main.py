...
def filter_child(
    ...
    return get_filter_provider().run(
        FilterRequest(audience="child", text=text, context=context, profile=profile)
    )
...
def filter_parent(
    text: str,
    context: FilterContext = "guidance",
    owner_id: str | None = None,
) -> FilterResponse:
    profile = profile_store.get_by_owner(owner_id) if owner_id else None
    return get_filter_provider().run(
        FilterRequest(audience="parent", text=text, context=context, profile=profile)
    )
    ...
    return [
        {
            ...
            "trigger": "All output",
            "what_it_does": "Reduces urgency, exclamations, chatter, and raw intensity before any specialist filter runs.",
        },
        {
            ...
            "always_active": False,
            "audience": "child",
            "trigger": "context=success or output_kind=praise_reinforcement",
            "what_it_does": "Softens praise amplitude and keeps reinforcement short, warm, and low-stimulation.",
        },
        {
            ...
            "always_active": False,
            "audience": "child + parent",
            "trigger": "retry, escalation, caregiver alerts, or frustration signals",
            "what_it_does": "Removes blame and pressure language from retry prompts and escalation-oriented messages.",
        },
        {
            ...
            "always_active": False,
            "audience": "child",
            "trigger": "context=reengagement or low engagement score",
            "what_it_does": "Shortens drifting-child prompts and replaces directive language with low-demand invitations.",
        },
        {
            ...
            "always_active": False,
            "audience": "parent",
            "trigger": "all parent-facing output, with extra shaping for caregiver alerts and environment guidance",
            "what_it_does": "De-alarms, de-jargons, and keeps parent-facing actions calm and practical.",
        },
        {
            "name": "architecture",
            "order": 6,
            ...
            "what_it_does": "Resolves an explicit output_kind, attaches its policy matrix, and reports whether the active strategy is rules_only or hybrid_rules_model.",
        },
    ]
        ...
        ctx: [
            {
                ...
                "embedding_source": p.embedding.source,
            }
            for p in phrases
        ]
...
def tone_best_match(child_id: str, context: PhraseContext) -> dict:
    ...
    if result is None:
        raise HTTPException(
            status_code=404,
            detail=f"No phrases available for context '{context}'.",
        )
    return {
    ...
    return [
        {
            ...
            "successful_phrase_ids": p.successful_phrase_ids,
            "overstimulation_flags": p.overstimulation_flags,
            "embedding_source": p.embedding_source,
            "last_updated": p.last_updated.isoformat(),
        }
        for p in tone_store.all_profiles()
    ]
